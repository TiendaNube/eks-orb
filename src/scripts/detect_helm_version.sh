...
# 
# Usage:
#   ./detect_helm_version.sh <release-name> <namespace>
#
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  # Get release name and namespace from environment variables or command line arguments
  RELEASE_NAME=${HELM_RELEASE_NAME:-$1}
  NAMESPACE=${HELM_NAMESPACE:-$2}
  ...
  if [ -z "$RELEASE_NAME" ] || [ -z "$NAMESPACE" ]; then
    ...
    echo "     HELM_RELEASE_NAME - The release name to check"
    echo "     HELM_NAMESPACE - The namespace to check"
